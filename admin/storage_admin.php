                    ...
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <div class="flex items-center gap-2">
                            <a href="addpost" type="button"
                                class="bg-green-100 text-green-800 rounded-md text-sm px-3 py-2 cursor-pointer">+
                                สร้างโพสต์ใหม่</a>
...
(() => {
    ...
    const openPost = (id) => {
        window.location.href = `post?id=${encodeURIComponent(String(id ?? ''))}`;
    };
    const goToEditPost = (id) => {
        window.location.href = `editpost?id=${encodeURIComponent(String(id ?? ''))}`;
    };
    ...
    const createActions = (id) => {
        ...
        btnEdit.type = 'button';
        btnEdit.className = 'text-blue-500 hover:underline';
        ...
        btnDelete.className = 'text-red-500 hover:underline';
    ...
    const createCard = (item) => {
        ...
        body.appendChild(createActions(id));
    ...
    const createRow = (item) => {
        ...
        row.className = 'bg-white rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition p-3 flex gap-3 items-center cursor-pointer';
        row.setAttribute('role', 'link');
        row.tabIndex = 0;
        row.addEventListener('click', () => openPost(id));
        ...
        actions.className = 'flex gap-3 items-center';
...
function deleteMedia(id) {
    Swal.fire('ยังไม่พร้อมใช้งาน', 'ส่วนสื่อการสอนยังไม่ได้เชื่อม API สำหรับลบ', 'info');
}
</script>