...
<div class="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow transition-transform ease-in-out duration-200">
   <h2 class="text-2xl font-bold mb-4">บทเรียน</h2>
  ...
  <div class="flex max-lg:flex-col justify-center max-md:flex-col gap-5 mb-2 mt-4">
    <?php foreach ($workshops as $i => $ws): ?>
      <div class="border border-purple-100 rounded-lg shadow-sm p-4 hover:shadow-lg hover:ring hover:ring-purple-300 w-full md:w-auto">
        <p class="text-center font-semibold text-lg text-violet-900">
          <?= htmlspecialchars($ws['name'] ?? ('Workshop ' . ($i+1))) ?>
        </p>

        <div class="flex text-sm my-3 flex-wrap">
          <span class="font-semibold text-purple-800">คำชี้แจง</span>
          <span class="pl-3">
            <?= htmlspecialchars($ws['desc'] ?? ('คำชี้แจงเกี่ยวกับ workshop ' . ($i+1) . '.. เวลา 3 ชั่วโมง')) ?>
          </span>
        </div>
        ...
        <div class="flex flex-col mt-4 mb-2 gap-2 w-full items-center">
          <?php foreach ($buttons as $btn): ?>
            <a href="<?= htmlspecialchars($btn['href'] ?? '#') ?>"
               class="w-1/2 lg:w-1/3 bg-purple-50 border border-purple-200 rounded-md py-1 px-4 transition-shadow transition-transform shadow-sm hover:bg-purple-200 hover:shadow-lg hover:-translate-y-0.5 ease-in-out duration-200 text-center">
